...
class SignalHmacSha256UpdateSpec(Contract):
    ...
    def specification(self) -> None:
        context           = self.alloc(signal_context_ty, read_only=True)
        hmac_context_data = self.fresh_var(array_ty(HMAC_CONTEXT_LENGTH, i8), "hmac_context_data")
        hmac_context      = self.alloc(array_ty(HMAC_CONTEXT_LENGTH, i8), points_to=hmac_context_data)
...
class SignalMessageGetMacSpec(Contract):
    ...
    def specification(self) -> None:
        ...
        mac_key_data                                           = self.fresh_var(array_ty(self.mac_key_len, i8), "mac_key_data")
        mac_key                                                = self.alloc(array_ty(self.mac_key_len, i8), points_to=mac_key_data)
        serialized_data                                        = self.fresh_var(array_ty(self.serialized_len, i8), "serialized_data")
        serialized                                             = self.alloc(array_ty(self.serialized_len, i8), points_to=serialized_data)
...
class SignalMessageVerifyMacSpec(Contract):
    ...
    def specification(self) -> None:
        ...
        self.points_to(elem(serialized, 8),                          serialized_message_data,                check_target_type = None)
        self.points_to(elem(serialized, 8 + serialized_message_len), expected_mac_data,                      check_target_type = None)

        base_message = struct(base, message_type, global_context, serialized)
        self.points_to(field(message, "base_message"),    base_message)
        self.points_to(field(message, "message_version"), int_to_8_cryptol(message_version))
